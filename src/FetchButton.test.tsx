...
test('it should make a network request to a URL passed as props', () => {
  ...
  expect(fetchSpy).toHaveBeenCalledWith(url, { signal: new AbortController().signal});
...
test('It should show the "Working" state for the duration of the network request', async () => {
  ...
  launchButton = await screen.findByText(/Launch Rocket/i, undefined, { timeout: 3000 });  // <- does not act as expected
...
test('It should optionally timeout the network request after a max duration passed as props', async () => {
  ...
  const fetchSpy = jest.spyOn(global, 'fetch')
    .mockImplementation((url, options) => new Promise(() => {
      // Will never respond
...
test('It should be possible to put the button into each state via props', () => {
  render(<Button state='inactive' tooltipText='Hello world' children="Press me" />);
  const inactiveButton = screen.getByTestId('button-inactive');
  expect(inactiveButton).toBeInTheDocument();

  render(<Button state='error' tooltipText='Error message' children="Press me" />);
  const errorButton = screen.getByTestId('button-error');
  expect(errorButton).toBeInTheDocument();

  render(<Button state='working' tooltipText='Error message' children="Press me" />);
...
test('The tooltip should not show if the button is disabled', () => {
  ...
  render(<Button state='error' tooltipText={erorrMessage} disabled={true} children="Press me" />);
  expect(screen.queryByText(erorrMessage)).not.toBeInTheDocument();

  let tooltipMessage = 'Busy';
  render(<Button state='working' tooltipText={tooltipMessage} disabled={true} children="Press me" />);
  expect(screen.queryByText(tooltipMessage)).not.toBeInTheDocument();

  tooltipMessage = 'Press me!';
  render(<Button state='inactive' tooltipText={tooltipMessage} disabled={true} children="Press me" />);
  expect(screen.queryByText(tooltipMessage)).not.toBeInTheDocument();
});
...
test('The error state should not show if the button is displayed or working', () => {
  ...
  render(<Button state='error' tooltipText={errorMessage} disabled={true} children="Press me" />);
  expect(screen.queryByText(errorMessage)).not.toBeInTheDocument();
});

test('The tooltip should alwasys show when in the error state', () => {
  const errorMessage = 'Error message';
  render(<Button state='error' tooltipText={errorMessage} children="Press me" />);
  const errorButton = screen.getByTestId('button-error');
  expect(errorButton).toBeInTheDocument();
